...
public class AutoLineFeedLayout extends ViewGroup {
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		if (DEBUG)
			Log.d(TAG, "--- onLayout changed :" + changed + " l :" + l + ",t :"
					+ t + ",r :" + r + ",b :" + b);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (DEBUG)
				Log.v(TAG, "childWidth :" + childWidth + " childHeight :"
						+ childHeight);
			...
			if (DEBUG)
				Log.d(TAG, "measure child :" + startOffsetX + ", "
						+ startOffsety + ", " + preEndOffsetX + ", "
						+ (startOffsety + childHeight));
			listX.add(startOffsetX);
			listY.add(startOffsety);

			// childView.layout(startOffsetX, startOffsety, preEndOffsetX,
			// startOffsety+childHeight);
		...
		setMeasuredDimension(measureWidth(widthMeasureSpec), startOffsety
				+ lastLineHeight + paddingBottom);
