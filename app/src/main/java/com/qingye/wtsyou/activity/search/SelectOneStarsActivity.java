...
public class SelectOneStarsActivity extends BaseHttpRecyclerActivity <EntityStarsItem,SelectOneStarsView,SelectOneStarsAdapter> implements View.OnClickListener, View.OnLongClickListener, OnBottomDragListener {
    ...
    public static Intent createIntent(Context context,List<EntityStars> selectStars) {
        ...
        return new Intent(context, SelectOneStarsActivity.class).putExtras(bundle);
    }
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        //此处仅能选择一个明星
        for (int i = 0;i < starsItem.size();i ++) {
            if (i == position) {
                starsItem.get(i).setSelector(true);
            } else {
                starsItem.get(i).setSelector(false);
            }
        }
        //因为只选择一个，所以先清空
        MapselectedStars.clear();
        selectedStars.clear();
        //选择的明星
        ...
        selectedStars.add(starsItem.get(position).getEntityStars());
        selectedStars(selectedStars);
